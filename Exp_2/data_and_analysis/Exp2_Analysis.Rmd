---
title: "Altering the preference for stable vs. unstable causes (introducing mean strength differences)"
output:  
  html_document:
...
```{r}
tdata <- read_delim("Exp2_data.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
...
tdata_long$causal_strength <- factor(tdata_long$causal_strength, levels = c("ninety", "ten"), 
                               labels = c("Unstable: Strength = 0.90 \nStable: Strength = 0.85", 
                                          "Unstable: Strength = 0.10 \nStable: Strength = 0.15"))
...
#tdata_long <- subset(tdata_long, causal_strength == "mean strength = 0.90")

#tdata_long <- subset(tdata_long, decision_level == "general norm")
...
a1 <- aov_car(main_dv_rec ~ decision_level*causal_strength + Error(run_id), tdata_sub)
a1
...
# get means and sds
m1 <- dat %>%
          filter(causal_strength == "Unstable: Strength = 0.90 \nStable: Strength = 0.85" & decision_level == "general norm")%>%
...
n1 <-  nrow(subset(dat, causal_strength == "Unstable: Strength = 0.90 \nStable: Strength = 0.85" & decision_level == "general norm"))


m2 <- dat %>%
          filter(causal_strength == "Unstable: Strength = 0.90 \nStable: Strength = 0.85" & decision_level == "personal")%>%
...
n2 <-  nrow(subset(dat, causal_strength == "Unstable: Strength = 0.90 \nStable: Strength = 0.85" & decision_level == "personal"))
...
med1 <- dat %>%
          filter(causal_strength == "Unstable: Strength = 0.90 \nStable: Strength = 0.85" & decision_level == "general norm")%>%
          summarize(Med1 = median(main_dv_rec))

med2 <- dat %>%
          filter(causal_strength == "Unstable: Strength = 0.90 \nStable: Strength = 0.85" & decision_level == "personal")%>%
          summarize(Med2 = median(main_dv_rec))
...
# get means and sds
m1 <- dat %>%
          filter(causal_strength == "Unstable: Strength = 0.10 \nStable: Strength = 0.15" & decision_level == "general norm")%>%
          summarize(Mean1 = mean(main_dv_rec))
...
n1 <-  nrow(subset(dat, causal_strength == "Unstable: Strength = 0.10 \nStable: Strength = 0.15" & decision_level == "general norm"))


m2 <- dat %>%
          filter(causal_strength == "Unstable: Strength = 0.10 \nStable: Strength = 0.15" & decision_level == "personal")%>%
          summarize(Mean2 = mean(main_dv_rec))
...
n2 <-  nrow(subset(dat, causal_strength == "Unstable: Strength = 0.10 \nStable: Strength = 0.15" & decision_level == "personal"))
...
CI_gov_Ind90_min <- t$conf.int[1]
CI_gov_Ind90_max <- t$conf.int[2]
t
...
t <- prop.test(cases[3],N_gov/2,correct=FALSE) # get CI for Invariance government
...
t <- prop.test(cases[5],N_pers/2,correct=FALSE) # get CI for Invariance government
...
t <- prop.test(cases[7],N_gov/2,correct=FALSE) # get CI for Invariance government
...
t <- prop.test(cases[9],N_pers/2,correct=FALSE) # get CI for Invariance government
...
CI_low <- c(CI_gov_Ind90_min, CI_gov_St90_min, CI_gov_USt90_min, CI_pers_St90_min, CI_pers_USt90_min,
            CI_gov_St10_min, CI_gov_USt10_min, CI_pers_St10_min, CI_pers_USt10_min)


CI_up <- c(CI_gov_Ind90_max, CI_gov_St90_max, CI_gov_USt90_max, CI_pers_St90_max, CI_pers_USt90_max,
...
g<- ggplot(plotdata, 
  ...
  scale_fill_manual(values=alpha(c("#636363", "#fc9272", "#3182bd"),0.7)) +
  labs(y = "Percentage", 
       fill = "Preference",
