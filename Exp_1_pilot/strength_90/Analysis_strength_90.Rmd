...
```{r}
tdata <- read.delim("pilot_data_strength_90.txt", header=TRUE, sep="\t", na.strings="NA", dec=".", strip.white=TRUE)
...
tdata_long$Strength <- factor(tdata_long$Strength, levels = c("ninety"), 
                              labels = c("0.90"))
...
g <- ggplot(tdata_sub, aes(x=variable, y=valueJitter, group = sID)) +
  ...
  geom_violinhalf(aes(y = value, group = variable, fill = variable), color = NA, position=position_dodge(1), alpha = 0.4)+
...
CI_low <- c(CI_gov_In_min, CI_gov_St_min, CI_gov_USt_min, CI_pers_St_min, CI_pers_USt_min)
CI_up <- c(CI_gov_In_max, CI_gov_St_max, CI_gov_USt_max, CI_pers_St_max, CI_pers_USt_max)
...
g<- ggplot(plotdata, 
  ...
  labs(y = "Percentage", 
       ...
       title = "Average causal strength = 0.90")+
